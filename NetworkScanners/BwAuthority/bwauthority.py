...
def main(argv):
    ...
    while True:
        ...
        if (p.returncode == 0):
            ...
            plog('NOTICE', 'restarting slice_num %d' % slice_num)
            ...
            # if uncaught exceptions are raised in user-written handlers,
            # TorCtl will kill the bwauthority_child process using
            # os.kill() because sys.exit() only exits the thread in which
            # the exception is caught.
            # quote mikeperry: "we want this thing not do die. that is
            # priority one" therefore: we restart the child process and
            # hope for the best :-)
...
if __name__ == '__main__':
    ...
    try:
        ...
        plog('ERROR', "An unexpected error occured: %d " % e)
